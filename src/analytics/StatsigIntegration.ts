...
export class StatsigIntegration {
  ...
  private deploymentThreshold: number = 0.95;
   ...
   */
  async createExperiment(opportunity: Opportunity): Promise<ExperimentConfig> {
    if (!opportunity.type) {
   ...
   */
  async createExperiments(opportunities: Opportunity[]): Promise<ExperimentConfig[]> {
   ...
   */
  async getExperimentResults(experimentName: string): Promise<ExperimentResult> {
    if (!this.initialized) {
      await this.initialize();
    }
   ...
   */
  async getExperimentConfig(experimentName: string): Promise<ExperimentConfig | null> {
   ...
   * Stop experiment
   */
  async stopExperiment(experimentName: string): Promise<boolean> {
   ...
   * Archive experiment
   */
  async archiveExperiment(experimentName: string): Promise<boolean> {
   ...
   */
  async rollback(experimentName: string, reason?: string): Promise<boolean> {
    if (!this.initialized) {
      await this.initialize();
    }
   ...
   */
  async deployWinner(experimentName: string): Promise<DeploymentResult> {
   ...
   */
  private generateExperimentName(type: OpportunityType): string {
