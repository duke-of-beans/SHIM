...
export class ChatCoordinator {
   ...
   */
  async registerChat(chatId: string): Promise<ChatSession> {
   ...
   */
  async unregisterChat(chatId: string): Promise<void> {
  ...
  async updateTaskProgress(
    taskId: string,
    update: { status: TaskStatus; progress?: number; message?: string }
  ): Promise<void> {
   ...
   * Complete task
   */
  async completeTask(taskId: string, completion: { success: boolean; result: any }): Promise<void> {
    const task = this.tasks.get(taskId);
    if (!task) {
      return;
    }
   ...
   * Get crashed chats
   */
  async getCrashedChats(): Promise<ChatSession[]> {
    const crashed = await this.registry.getCrashedWorkers();
    const crashedSessions: ChatSession[] = [];
   ...
   */
  async recoverCrashedTasks(): Promise<void> {
   ...
   */
  async aggregateResults(taskIds: string[]): Promise<any> {
    const results: any[] = [];
    let totalIssues = 0;
   ...
   */
  private findAvailableChat(): ChatSession | null {
   ...
   */
  private async assignNextQueuedTask(): Promise<void> {
