...
export class ResumeDetector {
   ...
   */
  async checkResume(sessionId: string): Promise<ResumeDetection> {
   ...
   */
  private formatSituation(reason: InterruptionReason): string {
    switch (reason) {
      case 'crash':
   ...
   * Format blockers
   */
  private formatBlockers(blockers: string[]): string {
