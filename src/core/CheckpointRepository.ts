...
interface CheckpointRow {
  ...
  restore_fidelity: number | null;
}

interface ResumeEventRow {
  id: string;
  checkpoint_id: string;
  session_id: string;
  restored_at: string;
  interruption_reason: string;
  time_since_checkpoint: number;
...
export class CheckpointRepository {
  ...
  private saveWithNumber(
    ...
    const compressedSize = 
      conversationStateCompressed.length +
  ...
  async getById(id: string): Promise<Checkpoint | null> {
    ...
    const row = await this.get<CheckpointRow>(sql, [id]);
  ...
  async getMostRecent(sessionId: string): Promise<Checkpoint | null> {
    ...
    const row = await this.get<CheckpointRow>(sql, [sessionId]);
    return row ? this.rowToCheckpoint(row) : null;
  }

  async listBySession(sessionId: string): Promise<Checkpoint[]> {
    const sql = `
      SELECT * FROM checkpoints 
      WHERE session_id = ? 
    ...
    const rows = await this.all<CheckpointRow>(sql, [sessionId]);
    return rows.map(row => this.rowToCheckpoint(row));
  }

  async getUnrestoredCheckpoint(sessionId: string): Promise<Checkpoint | null> {
    const sql = `
      SELECT * FROM checkpoints 
  ...
  async listByRisk(riskLevel: CrashRisk): Promise<Checkpoint[]> {
    const sql = `
      SELECT * FROM checkpoints 
      WHERE crash_risk = ? 
    ...
    const rows = await this.all<CheckpointRow>(sql, [riskLevel]);
    return rows.map(row => this.rowToCheckpoint(row));
  }

  async markRestored(id: string, success: boolean, fidelity: number): Promise<void> {
    const sql = `
      UPDATE checkpoints 
  ...
  async recordResumeEvent(event: ResumeEvent): Promise<void> {
  ...
  async getResumeEvents(sessionId: string): Promise<ResumeEvent[]> {
    const sql = `
    ...
    const rows = await this.all<ResumeEventRow>(sql, [sessionId]);
    return rows.map(row => ({
      id: row.id,
  ...
  async getCheckpointSize(id: string): Promise<CheckpointSize> {
    const sql = `
