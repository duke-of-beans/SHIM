...
class GoalReporter {
     ...
     */
    generateReport(goalId, tracker, format) {
        // Validate format
     ...
     */
    generateExecutiveSummary(goalId, tracker) {
        const progress = tracker.getProgress(goalId);
        let status = 'On track';
     ...
     */
    generateDetailedReport(goalId, tracker) {
        const progress = tracker.getProgress(goalId);
     ...
     */
    generateMultiGoalReport(goalIds, tracker) {
        if (goalIds.length === 0) {
     ...
     */
    generateTextReport(goalId, progress) {
     ...
     */
    generateMarkdownReport(goalId, progress) {
        let report = `# Goal Report: ${goalId}\n\n`;
        report += `**Goal ID:** ${goalId}\n\n`;
        report += `**Progress:** ${progress.completionPercentage}% (${progress.completedSubGoals}/${progress.totalSubGoals} completed)\n\n`;
        report += `**Velocity:** ${progress.velocity.toFixed(2)} sub-goals/hour\n\n`;
     ...
     */
    generateJSONReport(goalId, progress) {
        const report = {
            goalId,
            progress: {
                totalSubGoals: progress.totalSubGoals,
